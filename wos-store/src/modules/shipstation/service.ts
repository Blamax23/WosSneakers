...
import { AddressDTO, CalculateShippingOptionPriceDTO, CalculatedShippingOptionPrice, CartAddressDTO, CartLineItemDTO, OrderLineItemDTO, StockLocationAddressDTO } from "@medusajs/framework/types"
import { GetShippingRatesResponse, ShipStationAddress, Rate } from "./types"
import { ShipStationClient } from "./client"
import { LineItem } from "../../../.medusa/types/query-entry-points"
import { 
    FulfillmentOption,
...
class ShipStationProviderService extends AbstractFulfillmentProviderService {
    ...
    private async createShipment({
        ...
        items: CartLineItemDTO[] | OrderLineItemDTO[] // Remplace CartLineItemDTO / OrderLineItemDTO par LineItem
    ...
    async createFulfillment(
        ...
        items.map((item) => {
            const orderItem = (order as any)?.items?.find((i: any) => i.id === (item as any).line_item_id)
            if (!orderItem) {
                return
            }
        ...
        const newShipment = await this.createShipment({
            ...
            items: orderItemsToFulfill, // Utilisation de LineItem
    ...
    async getFulfillmentOptions(): Promise<FulfillmentOption[]> {
        ...
        console.log("Fulfillment Options:", fulfillmentOptions)
